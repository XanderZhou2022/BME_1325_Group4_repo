...
import { ICUDataOverlay } from '@/components/ICUDataOverlay';
import { ICUDashboard } from '@/components/ICUDashboard';
import { useICUData } from '@/hooks/useICUData';
...
export function MapViewer() {
  ...
  const { bedMonitors, mainConsole } = useICUData(6);
          ...
          {state.kind === 'ready' && state.layout.mapId === 'icu_layout' && (
            <div style={{ marginTop: '16px' }}>
              <button
                ...
                onClick={() => setViewMode(viewMode === 'dashboard' ? '3d' : 'dashboard')}
                style={{ width: '100%', padding: '8px', textAlign: 'center', cursor: 'pointer' }}
              >
                {viewMode === 'dashboard' ? '← Back to 3D View' : '📊 Open ICU Dashboard'}
              </button>
            </div>
          )}
              ...
              >
                {state.layout.mapId === 'icu_layout' && (
                  <ICUDataOverlay
                    ...
                    monitors={bedMonitors}
                    consoleData={mainConsole}
                    onConsoleClick={() => setViewMode('dashboard')}
                  />
                )}
              </ThreeFloorPlan>
            )
