...
export function ICUDataOverlay({ layout, monitors, consoleData, onConsoleClick }: Props) {
  // 1. Identify the Main Console (Central Display)
  // Updated to look for the console at the center of the 4x5 grid
  const mainConsole = layout.equipment.find(e =>
    (e.type === 'computer' && e.tileX === 9 && e.tileY === 9) ||
    e.type === 'diagnostic_table'
  );

  // 2. Bedside Monitors: Filter for 'bed' tiles representing patient stations
  ...
  return (
    <>
      {/* 1. Bedside Monitors */}
      {bedsideMonitors.map((eq) => {
        // Robust matching: Normalize IDs to handle format differences (e.g., "Bed_1" vs "BED-1" vs "1")
        const rawId = eq.equipmentId || eq.name || eq.id;
        const eqIdNorm = String(rawId || '').toLowerCase().replace(/[^a-z0-9]/g, '');

        const data = monitors.find(m => {
          const bedIdNorm = String(m.bedId || '').toLowerCase().replace(/[^a-z0-9]/g, '');
          if (!eqIdNorm || !bedIdNorm) return false;
          return eqIdNorm === bedIdNorm || eqIdNorm.includes(bedIdNorm) || bedIdNorm.includes(eqIdNorm);
        });

        // Fallback for debugging
        if (!data && monitors.length > 0) {
          console.log('ICU Overlay: No data match for', rawId, '. Available IDs:', monitors.map(m => m.bedId));
        }
            ...
            <div
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '8px' }}>
                <div style={{ background: '#1e293b', padding: '4px 8px', borderRadius: '4px' }}>
                  ...
                  <div style={{ fontWeight: 'bold' }}>{consoleData.totalPatients} / {consoleData.totalBeds || 8}</div>
                </div>
                <div style={{ background: '#1e293b', padding: '4px 8px', borderRadius: '4px' }}>
