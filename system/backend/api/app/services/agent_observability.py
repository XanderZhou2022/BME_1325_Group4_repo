...
def emit_agent_action_request_event(
    conn: Connection,
    *,
    admission_id: str,
    patient_id: str,
    ...
    """Emit agent_events for lab/MDT action requests (raised / started / completed / failed)."""
    event_id = new_id("aevt")
    with conn.cursor() as cur:
        cur.execute(
            """
            INSERT INTO agent_events (
                event_id, admission_id, patient_id, bed_id, producer_agent,
                event_type, schema_version, produced_at, output_id, payload
            ) VALUES (%s, %s, %s, %s, %s, %s, 'agent_action.v1', %s, NULL, %s::jsonb)
            """,
            (
                event_id,
                admission_id,
