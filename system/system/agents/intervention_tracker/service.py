...
def evaluate_intervention_tracker(conn: Connection, req: InterventionEvaluateRequest) -> InterventionEvaluateResponse:
    ...
    # urgency mapping (shared board):
    # responsive -> info
    # partially_responsive -> warning
    # non_responsive/deteriorating_despite_intervention -> critical
    resp_assessment = result["response_assessment"]
    if resp_assessment == "responsive":
        urgency = "info"
    elif resp_assessment == "partially_responsive":
        urgency = "warning"
    else:
        urgency = "critical"
    ...
    shared_entry = SharedBoardEntry(
        ...
        summary=result["escalation_hint"],
        evidence=result["evidence"],
        urgency=urgency,
        structured_payload={
            "intervention_type": intervention_type,
            "response_assessment": resp_assessment,
